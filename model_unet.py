...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, dropout_prob=0.2):
        super().__init__()
        self.bilinear = bilinear

        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleConv(in_channels, out_channels // 2, in_channels // 2, dropout_prob=dropout_prob)
        else:
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True, dropout_prob=0.2):
        ...
        self.inc = DoubleConv(n_channels, 64, dropout_prob=0.1)
